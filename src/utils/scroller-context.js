...
import scrollerData from './scroller-data'
import scrollerViewportChangeFunc from './viewport-change-func'
import {transformRpx as _transformRpx} from './transformRpx'
...
function ScrollContext({
  ...
  page._scrollerViewportChange = scrollerViewportChangeFunc
  this.comp = page.selectComponent('#' + id)
  this.itemSize = itemSize
  this.itemSizeOpt = itemSize
  // if (!this.comp) {
  // throw `<coolui-scroller> with id ${id} not found`
  // }
...
ScrollContext.prototype.checkComp = function () {
  if (!this.comp) {
    ...
    if (this.comp) {
      ...
      throw new Error('the coolui-scroller correspond to this context is detached, pls create another ScrollContext')
    }
  }
}
ScrollContext.prototype.appendList = function (list, cb) {
  this.checkComp()
  const id = this.id
  ...
  if (!scrollerData[id]) {
    ...
    scrollerData[id].dataKey = dataKey
    scrollerData[id].list = scrollerData[id].list.concat(list)
  }
  this._forceRerender(id, cb)
  return this
}
ScrollContext.prototype._forceRerender = function (id, cb) {
  ...
  if (Object.prototype.toString.call(this.itemSizeOpt) === '[object Object]' &&
    ...
    this._recalculateSizeByProp(scrollerData[id].list, function (sizeData) {
      scrollerData[id].sizeMap = sizeData.map
      scrollerData[id].sizeArray = sizeData.array
      // 触发强制渲染
      that.comp.forceUpdate(newcb)
    })
    return
  }
  const sizeData = this._recalculateSize(scrollerData[id].list)
  scrollerData[id].sizeMap = sizeData.map
...
ScrollContext.prototype._recalculateSizeByProp = function (list, cb) {
  ...
  if (itemSize.cacheKey) {
    ...
    // console.log('[coolui-scroller] get itemSize from cache', propValueMap)
  }
  ...
  if (propValueMap.length > 10) {
    // eslint-disable-next-line no-console
    console.warn('[coolui-scroller] get itemSize count exceed maximum of 10, now got', propValueMap)
  }
  ...
  function newItemSize(item, index) {
    ...
    if (!sizeIndex) {
      // eslint-disable-next-line no-console
      console.error('[coolui-scroller] auto calculate size array error, no map size found', item, index, item2PropValueMap)
      throw new Error('[coolui-scroller] auto calculate size array error, no map size found')
    }
    const size = propValueMap[sizeIndex.sizeIndex]
    if (!size) {
      // eslint-disable-next-line no-console
      console.log('[coolui-scroller] auto calculate size array error, no size found', item, index, sizeIndex, propValueMap)
      throw new Error('[coolui-scroller] auto calculate size array error, no size found')
    }
    return {
      width: size.width,
      height: size.height
...
ScrollContext.prototype._recalculateSize = function (list) {
  // 遍历所有的数据
  // 应该最多就千量级的, 遍历没有问题
  const sizeMap = {}
...
ScrollContext.prototype.deleteList = function (beginIndex, count, cb) {
  this.checkComp()
  const id = this.id
  if (!scrollerData[id]) {
    return this
  }
  scrollerData[id].list.splice(beginIndex, count)
  this._forceRerender(id, cb)
  return this
}
ScrollContext.prototype.updateList = function (beginIndex, list, cb) {
  this.checkComp()
  const id = this.id
  if (!scrollerData[id]) {
    return this
  }
  const len = scrollerData[id].list.length
  for (let i = 0; i < list.length && beginIndex < len; i++) {
    scrollerData[id].list[beginIndex++] = list[i]
  }
  this._forceRerender(id, cb)
  return this
}
ScrollContext.prototype.update = ScrollContext.prototype.updateList
ScrollContext.prototype.splice = function (begin, deleteCount, appendList, cb) {
  this.checkComp()
  const id = this.id
  const dataKey = this.dataKey
  // begin是数组
  ...
  if (!scrollerData[id]) {
    ...
    scrollerData[id].dataKey = dataKey
    const list = scrollerData[id].list
...
ScrollContext.prototype.append = ScrollContext.prototype.appendList

ScrollContext.prototype.destroy = function () {
  if (this.useInPage) {
  ...
  this.page = null
  this.comp = null
  if (scrollerData[this.id]) {
    delete scrollerData[this.id]
  }
  return this
}
// 重新更新下页面的数据
ScrollContext.prototype.forceUpdate = function (cb, reinitSlot) {
  this.checkComp()
  if (reinitSlot) {
    this.comp.reRender(() => {
...
ScrollContext.prototype.getBoundingClientRect = function (index) {
  this.checkComp()
  if (!scrollerData[this.id]) {
    return null
  }
  const sizeArray = scrollerData[this.id].sizeArray
...
ScrollContext.prototype.getScrollTop = function () {
  this.checkComp()
  return this.comp.currentScrollTop || 0
}
// 将px转化为rpx
ScrollContext.prototype.transformRpx = ScrollContext.transformRpx = function (str, addPxSuffix) {
  if (typeof str === 'number') str += 'rpx'
  return parseFloat(_transformRpx(str, addPxSuffix))
}
ScrollContext.prototype.getViewportItems = function (inViewportPx) {
  this.checkComp()
  const indexes = this.comp.getIndexesInViewport(inViewportPx)
  if (indexes.length <= 0) return []
  const viewportItems = []
  const list = scrollerData[this.id].list
  for (let i = 0; i < indexes.length; i++) {
    viewportItems.push(list[indexes[i]])
  }
  return viewportItems
}
ScrollContext.prototype.getTotalHeight = function () {
  this.checkComp()
  return this.comp.getTotalHeight()
}
// 返回完整的列表数据
ScrollContext.prototype.getList = function () {
  if (!scrollerData[this.id]) {
    return []
  }
  return scrollerData[this.id].list
}
