...
export default function (e, cb) {
  ...
  let newList = []
  const item = scrollerData[detail.id]
  // 边界值判断, 避免造成异常, 假设先调用了createScrollContext, 然后再延迟2s调用append插入数据的情况
  ...
  const obj = {
    // batchSetScrollerData: !this.data.batchSetScrollerData
  }
  obj[item.key] = newList
  const comp = this.selectComponent('#' + detail.id)
  obj[comp.data.batchKey] = !this.data.batchSetScrollerData
  ...
  // Fix #1
  // 去掉了batchSetDataKey，支持一个页面内显示2个coolui-scroller
  // const groupSetData = () => {
  //   this.setData(obj)
  //   comp._scrollerInnerBatchDataChanged(() => {
  //     if (typeof cb === 'function') {
  //       cb()
  //     }
  //   })
